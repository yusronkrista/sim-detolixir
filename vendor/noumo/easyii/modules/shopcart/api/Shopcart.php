...
class Shopcart extends \yii\easyii\components\API
{
    ...
    public function api_form($options = [])
    {
        ...
        echo $form->field($model, 'name')->textInput(['placeholder' => 'Isi nama lengkap Anda']);
        echo $form->field($model, 'address')->textInput(['placeholder' => 'Isi alamat lengkap Anda']);

        if($settings['enableEmail']) echo $form->field($model, 'email')->textInput(['placeholder' => 'Isi alamat email Anda']);
        if($settings['enablePhone']) echo $form->field($model, 'phone')->textInput(['placeholder' => 'Isi nomor telepon Anda']);

		echo $form->field($model, 'date')->widget(DatePicker::className(), [
		'dateFormat' => 'MMM d, yyyy',
		'clientOptions' => ['minDate' => 1],
		'options' => ['class' => 'form-control', 'placeholder' => 'Minimal satu hari setelah hari ini', 'readonly' => '']
		]);

        echo $form->field($model, 'comment')->textarea(['placeholder' => 'Isi tanggapan / pesan Anda']);
        echo $form->field($model, 'payment')->hiddenInput(['value' => 'Belum dibayar'])->label(false);

        echo Html::submitButton(Yii::t('easyii', 'Checkout'), ['class' => 'btn btn-primary']);
        ActiveForm::end();
